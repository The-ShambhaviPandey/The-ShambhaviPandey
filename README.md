# Shambhavi Pandey

Hi 👋, I'm **Shambhavi Pandey**, a final year B.Tech student in Computer Science and Engineering.  

I’m passionate about **Full-Stack Development, AI/ML, and Cybersecurity** and love building projects that **solve real-world problems**. Always open to **collaborating on interesting ideas**.  

 Noida, UP, India 

---

##  Tech Stack

**Languages:** Java | JavaScript | Python | C | TypeScript | HTML | CSS  
**Frameworks / Libraries:** React | Node.js | Express.js | Tailwind | Redux | Docker | Material-UI | AWS | GCP  
**Database / Tools:** MongoDB | Redis | Postman | Multer | JWT | Figma | VSCode  

**Focus:** Full-Stack Development, Gen AI Projects

---

##  Achievements

- 🥇 Winner, Cybersecurity Hackathon – CDAC Noida, 2025  
- 🥇 Winner, IPEC-TBI Hackathon – Bharat Shiksha Expo, 2025  
- 🏅 Grand Finalist, India AI Impact Hackathon – HCL GUVI, 2025  
- 🏅 Participant, Google Gen AI Hackathon  

---

## GitHub Stats

![Streak](https://github-readme-streak-stats.herokuapp.com/?user=The-ShambhaviPandey&theme=dark)

---

 ## Thank you! 💙
 
 You can connect with me [here](https://www.linkedin.com/in/theshambhavipandey/)


